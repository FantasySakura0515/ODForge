...
- `--theme`：內建主題（僅對 `.odp` 有效），十三選一：`academic`、`minimal`、`dark`、
  ...
  省略則尊重 LLM 自己定的美術方向。
- `--style classic | report | academic | keynote | editorial | zen`：版式
  （僅對 `.odp` 有效）；省略則使用該主題配對的預設版式。
...
左側常駐導覽有「工作紀錄」與「範本庫」兩個地方，範本庫是自己的一頁（`?templates=1`）。
一個範本 = **版式 × 配色**：

- **版式**決定構圖：封面怎麼排、內頁標題用什麼記號、分節頁怎麼處理、頁尾留多少家具。
  六套內建，每一套都照著一個公開、被廣泛使用的慣例做，不是自己發明的：

  | 版式 | 依據 | 長相 |
  |---|---|---|
  | `classic` 學院派 | ODForge 原本的版面 | 置中封面＋點陣、標題左側色條、滿版分節頁 |
  | `report` 顧問報告 | MBB（McKinsey／BCG／Bain）簡報結構 | 左上 action title、標題下一條細線、出處與頁碼在頁尾 |
  | `academic` 學術簡潔 | Beamer [metropolis](https://github.com/matze/mtheme) 主題 | 標題下 0.4pt 分隔線、底部進度條、分節頁上下夾兩條線 |
  | `keynote` 舞台 | 滿版出血 + 下三分之一錨定文字 | 封面整頁強調色、內頁只有大字、只留頁碼 |
  | `editorial` 編輯風 | Swiss／國際主義平面風格 | 頂部通欄細線、粗標題橫貫中段、超大淡編號分節頁 |
  | `zen` 極簡 | 極簡／設計導向版面 | 沒有線、沒有頁尾；標題落在上三分之一 |

  這一層是「兩份範本真的是兩份文件」的來源；只換顏色不換版式，十三套就只是同一個版面
  的十三種配色。
